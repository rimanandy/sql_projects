...
pizzas on pizza_types.pizza_type_id=pizzas.pizza_type_id order by
...
join order_details on 
order_details.pizza_id= pizzas.pizza_id group by pizza_types.name
...
select round(avg(quantity),0) as average_quantity from
...
round(sum(order_details.quantity* pizzas.price)/(select round(sum(
order_details.quantity * pizzas.price),2) as total_sales from
